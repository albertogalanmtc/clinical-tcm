...
export function PrescriptionPreviewModal({ 
  ...
  const ui = {
    title: isSpanish ? 'Vista previa de prescripción' : 'Prescription Preview',
    description: isSpanish ? 'Ver todos los detalles de la prescripción' : 'View complete prescription details',
  ...
  const activeSafetyFilters = prescription.patientSafetyProfile 
    ? Object.entries(prescription.patientSafetyProfile)
        ...
        .map(([key, _]) => formatSafetyLabel(key, language))
    : [];
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed inset-x-0 bottom-0 top-[10vh] sm:left-1/2 sm:top-1/2 sm:right-auto sm:bottom-auto sm:-translate-x-1/2 sm:-translate-y-1/2 bg-white rounded-t-2xl sm:rounded-lg sm:max-w-3xl w-full sm:max-h-[90vh] overflow-hidden z-[80] flex flex-col">
          <Dialog.Description className="sr-only">
            {isSpanish ? `Vista previa detallada de la prescripción ${prescription.prescription_id}` : `Detailed preview of prescription ${prescription.prescription_id}`}
          </Dialog.Description>
            ...
            <div className="flex-1 overflow-y-auto p-6">
              ...
              <div className="mb-6">
                ...
                <p className="text-sm text-gray-500">
                  {ui.createdOn} {new Date(prescription.createdAt).toLocaleDateString(isSpanish ? 'es-ES' : 'en-US')}
                </p>
                {prescription.createdBy && (
                  <p className="text-sm text-gray-600 mt-1">
                    {ui.by} {prescription.createdBy.userName}
                  </p>
                )}
              </div>
              ...
              {prescription.alertMode === 'filtered' && activeSafetyFilters.length > 0 && (
                <div className="mb-8">
                  <h2 className="text-xl font-semibold text-gray-900 mb-4">{ui.patientSafetyProfile}</h2>
              ...
              {prescription.comments && (
                <div className="mb-8">
                  <h2 className="text-xl font-semibold text-gray-900 mb-4">{ui.comments}</h2>
                  <div className="bg-gray-50 rounded-lg p-4">
              ...
              {prescription.components.length === 0 && (
                <div className="text-center py-8">
                  <p className="text-gray-500">{ui.noComponents}</p>
                </div>
              )}
            </div>
...
function formatSafetyLabel(key: string, language: 'en' | 'es'): string {
  const isSpanish = language === 'es';
  const labels: Record<string, string> = isSpanish
    ? {
        ...
        insomnia: 'Insomnio',
        epilepsy: 'Epilepsia',
        bleeding_disorders: 'Trastornos hemorrágicos',
