...
export function FormulaJsonEditorModal({ 
  formula, 
  onClose,
  onSave 
}: { 
  ...
  const [jsonText, setJsonText] = useState(() => JSON.stringify(formula, null, 2));
  ...
  const handleSave = () => {
    try {
      ...
      // Basic validation
      if (!parsed.formula_id || !parsed.pinyin_name) {
        setError('Required fields missing: formula_id and pinyin_name are required');
        return;
      }

      setError(null);
      onSave(parsed as Formula);
      onClose();
    } catch (e) {
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed inset-x-0 bottom-0 top-[10vh] sm:left-1/2 sm:top-1/2 sm:right-auto sm:bottom-auto sm:-translate-x-1/2 sm:-translate-y-1/2 bg-white rounded-t-2xl sm:rounded-lg sm:max-w-4xl w-full sm:max-h-[90vh] overflow-hidden z-[80] flex flex-col">
          <Dialog.Description className="sr-only">
            JSON editor for {formula.pinyin_name} formula data
          </Dialog.Description>
          <div className="flex flex-col h-full">
              ...
              <div className="flex items-start justify-between">
                <div>
                  <Dialog.Title className="text-xl font-bold text-gray-900">
                    Edit Formula JSON: {formula.pinyin_name}
                  </Dialog.Title>
                  <Dialog.Description className="text-sm text-gray-600 mt-1">
                    Modify the formula data in JSON format. Changes will be saved to the library.
                  </Dialog.Description>
                </div>
            ...
            <div className="flex-shrink-0 px-6 py-4 border-t border-gray-200 bg-gray-50 flex items-center justify-between">
              <div className="text-sm text-gray-500">
                <span className="font-medium">Formula ID:</span> {formula.formula_id}
              </div>
