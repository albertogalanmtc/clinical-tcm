...
export default function AccountLayout() {
  ...
  // Check if we're on the Account index page or a subpage
  const isAccountIndex = location.pathname === '/account' || location.pathname === '/account/';
  const isOnSubPage = !isAccountIndex;
  ...
  const handleMobileBack = () => {
    if (isOnSubPage) {
      // If on a subpage, go back to Account index
      navigate('/account');
    } else {
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex flex-col flex-1 min-h-0 overflow-hidden lg:overflow-auto p-0 lg:p-6 lg:pb-6 gap-0 lg:gap-4 bg-white lg:bg-transparent">
        ...
        <div className="flex gap-4 lg:gap-6 flex-1 min-h-0 items-stretch">
          ...
          <main className={`
            ${isAccountIndex ? 'hidden lg:flex' : 'flex'}
            flex-1 min-w-0 bg-white lg:rounded-lg lg:border border-gray-200 lg:max-h-[calc(100vh-7rem)] flex-col overflow-hidden
            fixed lg:static top-14 left-0 right-0 ${isAccountIndex || isModalOpen ? 'bottom-[68px]' : 'bottom-0'} lg:h-auto z-[40]
          `}>
